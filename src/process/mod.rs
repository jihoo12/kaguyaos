...
extern "C" fn preempt_stress_worker() {
    ...
    let mut checksum = task_id as u64 ^ 0x9e37_79b9_7f4a_7c15;

    // Keep interrupts enabled: the point of this probe is to force repeated
    // timer-driven Running -> SwitchingOut handoffs on both CPUs.
    for round in 0..12_000_000usize {
        checksum = checksum
            .rotate_left(7)
            .wrapping_add((round as u64) ^ 0xa5a5_5a5a_d3c1_b7e9);
        core::hint::black_box(checksum);
    }

    let done = PREEMPT_STRESS_DONE.fetch_add(1, Ordering::AcqRel) + 1;
    crate::println!(
        "[sched-preempt-stress] task {} done ({}/{}) checksum={:#x}",
        task_id,
        done,
        PREEMPT_STRESS_WORKERS,
        checksum
    );
    terminate_task(0);
...
/// Temporary validation probe for the outgoing-context handoff. Call once after
/// the normal scheduler is initialized, then remove before merging.
pub fn start_preempt_stress_probe() {
    PREEMPT_STRESS_DONE.store(0, Ordering::Release);
    crate::println!(
        "[sched-preempt-stress] starting {} workers with timer preemption enabled",
        PREEMPT_STRESS_WORKERS
    );
...
/// Print PASS once all temporary preemption workers have terminated.
pub fn service_preempt_stress_probe() {
    static REPORTED: AtomicBool = AtomicBool::new(false);
    if PREEMPT_STRESS_DONE.load(Ordering::Acquire) == PREEMPT_STRESS_WORKERS
        && !REPORTED.swap(true, Ordering::AcqRel)
    {
        crate::println!(
            "[sched-preempt-stress] PASS: {}/{} workers completed",
            PREEMPT_STRESS_WORKERS,
            PREEMPT_STRESS_WORKERS
        );
    }
